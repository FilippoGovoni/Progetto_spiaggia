...
int main(){
    ...
    fgets(dato.Data,15,stdin);
    t=strlen(dato.Data);
    dato.Data[t-1]='\0';


    // Creazione thread per la gestione dei file
    /*if(pthread_create(&writer,NULL,filewriter,NULL)<0)
       {
...
void *gestore_client(void *dato){
	...
	if( (read_size = recv(dati.sock,MSG.message,100,0))>0){
		lenght=strlen(MSG.message);
        MSG.message[lenght]='\0';

        if(strncmp(MSG.message,"BOOK",4)==0)
            controllo=0;
        if(strncmp(MSG.message,"CANCEL",6)==0)
            controllo=1;
        if(strncmp(MSG.message,"AVAILABLE",9)==0)
            controllo=2;
        switch(controllo){
            case 0://BOOK
            ...
            break;
            case 1://CANCEL
            func_CANCEL(dati.sock,MSG.message,ombrellone);
            break;
            case 2: //AVAILABLE
            func_AVAILABLE(dati.sock,MSG.message,ombrellone);
            break;
...
void *filewriter(){	//gli si potrebbe passare statospiaggia come parametro ed ottimizzare un poco il codice
    int i,t;
    FILE* statospiaggia;
	FILE* modifiche;
	Ombrellone Supporto;
